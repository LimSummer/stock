...
cache1 = {}
def analyseData1(datalist,dec):
    print 'in analyseData1'
    try:
        for data in datalist:
            stockid = data[2]
            yesprice = float(data[4])
            #print stockid, yesprice
            if not cache1.has_key(stockid):
                cache1[stockid] = float(data[3])
            else:
                curprice = float(data[3])
                preprice = cache1[stockid]
                print curprice,preprice
                if (abs(curprice - preprice) / yesprice) * 100 > 0:
                    logging.info(stockid)
                cache1[stockid] = float(data[3])
